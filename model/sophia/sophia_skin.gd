extends CharacterBody3D

@onready var animation_tree = %AnimationTree
@onready var state_machine : AnimationNodeStateMachinePlayback = animation_tree.get("parameters/StateMachine/playback")
@onready var move_tilt_path : String = "parameters/StateMachine/Move/tilt/add_amount"

var run_tilt = 0.0 : set = _set_run_tilt

@export var blink = true : set = set_blink
@onready var blink_timer = %BlinkTimer
@onready var closed_eyes_timer = %ClosedEyesTimer
@onready var eye_mat = $sophia/rig/Skeleton3D/Sophia.get("surface_material_override/2")

const SPEED = 5.0 * 2

@onready var camera = $"../Camera3D"
@onready var sophia = $sophia

@onready var field = $"../../Dungeon"

func _ready():
	blink_timer.connect("timeout", func():
		eye_mat.set("uv1_offset", Vector3(0.0, 0.5, 0.0))
		closed_eyes_timer.start(0.2)
		)
		
	closed_eyes_timer.connect("timeout", func():
		eye_mat.set("uv1_offset", Vector3.ZERO)
		blink_timer.start(randf_range(1.0, 4.0))
		)
		
	camera.position = Vector3(global_position.x, global_position.y + 7, global_position.z + 5)

func set_blink(state : bool):
	if blink == state: return
	blink = state
	if blink:
		blink_timer.start(0.2)
	else:
		blink_timer.stop()
		closed_eyes_timer.stop()

func _set_run_tilt(value : float):
	run_tilt = clamp(value, -1.0, 1.0)
	animation_tree.set(move_tilt_path, run_tilt)

func idle():
	state_machine.travel("Idle")

func move():
	state_machine.travel("Move")

func fall():
	state_machine.travel("Fall")

func jump():
	state_machine.travel("Jump")

func edge_grab():
	state_machine.travel("EdgeGrab")

func wall_slide():
	state_machine.travel("WallSlide")

var current_position = Vector3(0.0, 0.0, 0.0)
var target_position = Vector3(0.0, 0.0, 0.0)
var is_move = false
var move_direction = 'up'

func _physics_process(delta: float) -> void:
	var input_action = Input.get_vector("left", "right", "up", "down")
	var input_direction = (transform.basis * Vector3(input_action.x, 0, input_action.y)).normalized()
	
	if is_move:
		var direction = target_position - global_position
		direction.y = 0.0
		if direction.length_squared() < 0.00001:
			is_move = false
			current_position = target_position
			
			if input_direction == Vector3(0.0, 0.0, 0.0) or (input_direction.x > 0 and move_direction != 'right') or (input_direction.x < 0 and move_direction != 'left') or (input_direction.z > 0 and move_direction != 'up') or (input_direction.z < 0 and move_direction != 'down'):
				velocity = Vector3(0.0, 0.0, 0.0)
				idle()
		else:
			direction = direction.normalized() * SPEED
			velocity.x = direction.x
			velocity.z = direction.z
	
	if !is_move:
		if input_direction.x > 0:
			target_position.x = current_position.x + 2
			sophia.rotation.y = 89.5
			move_direction = 'right'
		elif input_direction.x < 0:
			target_position.x = current_position.x - 2
			sophia.rotation.y = -89.5
			move_direction = 'left'
		elif input_direction.z > 0:
			target_position.z = current_position.z + 2
			sophia.rotation.y = 0
			move_direction = 'up'
		elif input_direction.z < 0:
			target_position.z = current_position.z - 2
			sophia.rotation.y = 91
			move_direction = 'down'
		
		if (input_direction != Vector3(0.0, 0.0, 0.0)):
			is_move = true
			move()
	
	# カメラ
	camera.position = Vector3(global_position.x, global_position.y + 7, global_position.z + 5)

	move_and_slide()
